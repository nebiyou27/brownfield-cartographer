...
class GraphNode(BaseModel):
    ...
    id: str = Field(..., description="Unique identifier for the node (e.g., file path or table name)")
    source_file: str = Field(..., description="The YAML or SQL file where this node is defined")
    source_line: Optional[int] = Field(None, description="Line number where the definition starts")
    description: Optional[str] = Field("", description="A human-readable description of this node")
...
class ModuleNode(GraphNode):
    ...
    logical_name: Optional[str] = Field(None, description="The logical dbt name of the resource defined in this file")
...
class DatasetNode(GraphNode):
    ...
    dataset_type: str = Field(..., description="The type of dataset (e.g., dbt model, raw source, seed file)")
    
    # The columns this dataset contains, if we can parse them from yaml or infer them.
    columns: List[str] = Field(default_factory=list, description="List of column names in this dataset")
    column_descriptions: Dict[str, str] = Field(default_factory=dict, description="Dictionary mapping column names to descriptions")
...
class TransformationEdge(BaseModel):
    ...
    """
    source_dataset: str = Field(..., description="The ID of the DatasetNode supplying data (upstream)")
    target_dataset: str = Field(..., description="The ID of the DatasetNode consuming data (downstream)")
    
    # Traceability
    source_file: str = Field(..., description="The file where this transformation/dependency was discovered")
    source_line: Optional[int] = Field(None, description="The line number where this dependency was found")
