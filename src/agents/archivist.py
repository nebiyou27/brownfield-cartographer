...
class Archivist:
    ...
    def archive(
        ...
        self._generate_codebase_md(module_graph, lineage_graph, semantic_results, git_velocity or {})
    ...
    def _generate_codebase_md(
        ...
        with open(path, "w", encoding="utf-8") as f:
            ...
            f.write(f"*Auto-generated by Brownfield Cartographer · {datetime.now().strftime('%Y-%m-%d')}*\n\n")
            f.write("> Inject this file as system-prompt context for instant architectural awareness.\n\n")
            f.write("---\n\n")
            ...
            f.write("*Grouped by inferred business domain. Purpose statements grounded in code, not docstrings.*\n\n")
            ...
            for domain in sorted(domains.keys()):
                ...
                for mod_id in sorted(set(domains[domain])):
                    ...
                    drift_data = (
                        drift_flags.get(mod_id)
                        or drift_flags.get(mod_id.replace("\\", "/"))
                    )
                    ...
                    if node_in_graph:
                        upstream = [
                            u for u, _ in g.in_edges(node_in_graph)
                            if not _is_pseudo(u)
                        ]
                        downstream = [
                            v for _, v in g.out_edges(node_in_graph)
                            if not _is_pseudo(v)
                        ]
                        if upstream:
                            f.write("**Upstream:** " + ", ".join(f"`{u}`" for u in sorted(upstream)) + "\n\n")
                        if downstream:
                            f.write("**Downstream:** " + ", ".join(f"`{v}`" for v in sorted(downstream)) + "\n\n")

                f.write("---\n\n")

        print(f"[Archivist] Generated CODEBASE.md → {path}")
    ...
    def _generate_audit_trace(
        ...
        with open(log_path, "w", encoding="utf-8") as f:
            ...
            failed = [
                nid for nid, attrs in lineage_graph.graph.nodes(data=True)
            ...
            drift = {k: v for k, v in semantic_results.get("drift_flags", {}).items() if not _is_pseudo(k)}
        ...
        print(f"[Archivist] Generated audit_trace.log → {log_path}")
